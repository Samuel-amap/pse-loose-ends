module generated_test_1234_16
using PlantSimEngine

PlantSimEngine.@process "p1" verbose = false
PlantSimEngine.@process "p2" verbose = false
PlantSimEngine.@process "p3" verbose = false
PlantSimEngine.@process "p4" verbose = false
PlantSimEngine.@process "p5" verbose = false
PlantSimEngine.@process "p6" verbose = false
PlantSimEngine.@process "p7" verbose = false
PlantSimEngine.@process "p8" verbose = false
PlantSimEngine.@process "p9" verbose = false
PlantSimEngine.@process "p10" verbose = false
PlantSimEngine.@process "p11" verbose = false
PlantSimEngine.@process "p12" verbose = false
PlantSimEngine.@process "p13" verbose = false
PlantSimEngine.@process "p14" verbose = false
PlantSimEngine.@process "p15" verbose = false
PlantSimEngine.@process "p16" verbose = false
PlantSimEngine.@process "p17" verbose = false
PlantSimEngine.@process "p18" verbose = false
PlantSimEngine.@process "p19" verbose = false

struct Model1_E1 <: AbstractP1Model
 end

struct Model2_E1 <: AbstractP2Model
 end

struct Model3_E3 <: AbstractP3Model
 end

struct Model4_E1 <: AbstractP4Model
 end

struct Model5_E5 <: AbstractP5Model
 end

struct Model6_E1 <: AbstractP6Model
 end

struct Model7_E7 <: AbstractP7Model
 end

struct Model8_E8 <: AbstractP8Model
 end

struct Model9_E1 <: AbstractP9Model
 end

struct Model10_E2 <: AbstractP10Model
 end

struct Model11_E1 <: AbstractP11Model
 end

struct Model12_E9 <: AbstractP12Model
 end

struct Model13_E8 <: AbstractP13Model
 end

struct Model14_E1 <: AbstractP14Model
 end

struct Model15_E6 <: AbstractP15Model
 end

struct Model16_E5 <: AbstractP16Model
 end

struct Model17_E11 <: AbstractP17Model
 end

struct Model18_E11 <: AbstractP18Model
 end

struct Model19_E4 <: AbstractP19Model
 end


function PlantSimEngine.inputs_(::Model1_E1)
	NamedTuple()
end
function PlantSimEngine.inputs_(::Model2_E1)
	NamedTuple()
end
function PlantSimEngine.inputs_(::Model3_E3)
	(a2_3 = -Inf, )
end
function PlantSimEngine.inputs_(::Model4_E1)
	NamedTuple()
end
function PlantSimEngine.inputs_(::Model5_E5)
	(a1_5 = -Inf, )
end
function PlantSimEngine.inputs_(::Model6_E1)
	NamedTuple()
end
function PlantSimEngine.inputs_(::Model7_E7)
	(a3_7 = -Inf, )
end
function PlantSimEngine.inputs_(::Model8_E8)
	(a4_8 = -Inf, )
end
function PlantSimEngine.inputs_(::Model9_E1)
	NamedTuple()
end
function PlantSimEngine.inputs_(::Model10_E2)
	(a6_10 = -Inf, )
end
function PlantSimEngine.inputs_(::Model11_E1)
	(a6_11 = -Inf, )
end
function PlantSimEngine.inputs_(::Model12_E9)
	(a6_12 = -Inf, )
end
function PlantSimEngine.inputs_(::Model13_E8)
	(a4_13 = -Inf, )
end
function PlantSimEngine.inputs_(::Model14_E1)
	NamedTuple()
end
function PlantSimEngine.inputs_(::Model15_E6)
	(a6_15 = -Inf, )
end
function PlantSimEngine.inputs_(::Model16_E5)
	NamedTuple()
end
function PlantSimEngine.inputs_(::Model17_E11)
	(a8_17 = -Inf, )
end
function PlantSimEngine.inputs_(::Model18_E11)
	(a8_18 = -Inf, )
end
function PlantSimEngine.inputs_(::Model19_E4)
	NamedTuple()
end

function PlantSimEngine.outputs_(::Model1_E1)
	(a1_5 = -Inf, out1 = -Inf, )
end
function PlantSimEngine.outputs_(::Model2_E1)
	(a2_3 = -Inf, out2 = -Inf, )
end
function PlantSimEngine.outputs_(::Model3_E3)
	(a3_7 = -Inf, out3 = -Inf, )
end
function PlantSimEngine.outputs_(::Model4_E1)
	(a4_8 = -Inf, a4_13 = -Inf, out4 = -Inf, )
end
function PlantSimEngine.outputs_(::Model5_E5)
	(out5 = -Inf, )
end
function PlantSimEngine.outputs_(::Model6_E1)
	(a6_10 = -Inf, a6_11 = -Inf, a6_12 = -Inf, a6_15 = -Inf, out6 = -Inf, )
end
function PlantSimEngine.outputs_(::Model7_E7)
	(out7 = -Inf, )
end
function PlantSimEngine.outputs_(::Model8_E8)
	(a8_17 = -Inf, a8_18 = -Inf, out8 = -Inf, )
end
function PlantSimEngine.outputs_(::Model9_E1)
	(out9 = -Inf, )
end
function PlantSimEngine.outputs_(::Model10_E2)
	(out10 = -Inf, )
end
function PlantSimEngine.outputs_(::Model11_E1)
	(out11 = -Inf, )
end
function PlantSimEngine.outputs_(::Model12_E9)
	(out12 = -Inf, )
end
function PlantSimEngine.outputs_(::Model13_E8)
	(out13 = -Inf, )
end
function PlantSimEngine.outputs_(::Model14_E1)
	(out14 = -Inf, )
end
function PlantSimEngine.outputs_(::Model15_E6)
	(out15 = -Inf, )
end
function PlantSimEngine.outputs_(::Model16_E5)
	(out16 = -Inf, )
end
function PlantSimEngine.outputs_(::Model17_E11)
	(out17 = -Inf, )
end
function PlantSimEngine.outputs_(::Model18_E11)
	(out18 = -Inf, )
end
function PlantSimEngine.outputs_(::Model19_E4)
	(out19 = -Inf, )
end

function PlantSimEngine.run!(::Model1_E1, models, status, meteo, constants=nothing, sim_object=nothing)
	status.a1_5 = 1.0
status.out1 = 1.0
end
function PlantSimEngine.run!(::Model2_E1, models, status, meteo, constants=nothing, sim_object=nothing)
	status.a2_3 = 1.0
status.out2 = 1.0
end
function PlantSimEngine.run!(::Model3_E3, models, status, meteo, constants=nothing, sim_object=nothing)
	status.a3_7 = 1.0+ status.a2_3
status.out3 = 1.0
end
function PlantSimEngine.run!(::Model4_E1, models, status, meteo, constants=nothing, sim_object=nothing)
	status.a4_8 = 1.0
	status.a4_13 = 1.0
status.out4 = 1.0
end
function PlantSimEngine.run!(::Model5_E5, models, status, meteo, constants=nothing, sim_object=nothing)
status.out5 = 1.0
end
function PlantSimEngine.run!(::Model6_E1, models, status, meteo, constants=nothing, sim_object=nothing)
	status.a6_10 = 1.0
	status.a6_11 = 1.0
	status.a6_12 = 1.0
	status.a6_15 = 1.0
status.out6 = 1.0
end
PlantSimEngine.dep(::Model7_E7) = (p16=AbstractP16Model => ("E5",),)
function PlantSimEngine.run!(::Model7_E7, models, status, meteo, constants=nothing, sim_object=nothing)
	status_E5 = sim_object.statuses["E5"][1]
	run!(sim_object.models["E5"].p16, models, status_E5, meteo, constants, sim_object)
status.out7 = 1.0
end
function PlantSimEngine.run!(::Model8_E8, models, status, meteo, constants=nothing, sim_object=nothing)
	status.a8_17 = 1.0+ status.a4_8
	status.a8_18 = 1.0+ status.a4_8
status.out8 = 1.0
end
function PlantSimEngine.run!(::Model9_E1, models, status, meteo, constants=nothing, sim_object=nothing)
status.out9 = 1.0
end
function PlantSimEngine.run!(::Model10_E2, models, status, meteo, constants=nothing, sim_object=nothing)
status.out10 = 1.0
end
PlantSimEngine.dep(::Model11_E1) = (p19=AbstractP19Model => ("E4",),)
function PlantSimEngine.run!(::Model11_E1, models, status, meteo, constants=nothing, sim_object=nothing)
	status_E4 = sim_object.statuses["E4"][1]
	run!(sim_object.models["E4"].p19, models, status_E4, meteo, constants, sim_object)
status.out11 = 1.0
end
function PlantSimEngine.run!(::Model12_E9, models, status, meteo, constants=nothing, sim_object=nothing)
status.out12 = 1.0
end
function PlantSimEngine.run!(::Model13_E8, models, status, meteo, constants=nothing, sim_object=nothing)
status.out13 = 1.0
end
function PlantSimEngine.run!(::Model14_E1, models, status, meteo, constants=nothing, sim_object=nothing)
status.out14 = 1.0
end
function PlantSimEngine.run!(::Model15_E6, models, status, meteo, constants=nothing, sim_object=nothing)
status.out15 = 1.0
end
function PlantSimEngine.run!(::Model16_E5, models, status, meteo, constants=nothing, sim_object=nothing)
status.out16 = 1.0
end
function PlantSimEngine.run!(::Model17_E11, models, status, meteo, constants=nothing, sim_object=nothing)
status.out17 = 1.0
end
function PlantSimEngine.run!(::Model18_E11, models, status, meteo, constants=nothing, sim_object=nothing)
status.out18 = 1.0
end
function PlantSimEngine.run!(::Model19_E4, models, status, meteo, constants=nothing, sim_object=nothing)
status.out19 = 1.0
end

mapping = Dict( "E1" => (
Model1_E1(),
Model2_E1(),
Model4_E1(),
Model6_E1(),
Model9_E1(),
Model11_E1(),
Model14_E1(),
Status(a1_5 = 1.0, a2_3 = 1.0, a4_8 = 1.0, a4_13 = 1.0, a6_10 = 1.0, a6_11 = 1.0, a6_12 = 1.0, a6_15 = 1.0, ),
),
"E2" => (
MultiScaleModel(
model = Model10_E2(),
mapped_variables=[:a6_10 => "E1" => :a6_10,]
),
Status(),
),
"E3" => (
MultiScaleModel(
model = Model3_E3(),
mapped_variables=[:a2_3 => "E1" => :a2_3,]
),
Status(a3_7 = 1.0, ),
),
"E4" => (
Model19_E4(),
Status(),
),
"E5" => (
MultiScaleModel(
model = Model5_E5(),
mapped_variables=[:a1_5 => "E1" => :a1_5,]
),
Model16_E5(),
Status(),
),
"E6" => (
MultiScaleModel(
model = Model15_E6(),
mapped_variables=[:a6_15 => "E1" => :a6_15,]
),
Status(),
),
"E7" => (
MultiScaleModel(
model = Model7_E7(),
mapped_variables=[:a3_7 => "E3" => :a3_7,]
),
Status(),
),
"E8" => (
MultiScaleModel(
model = Model8_E8(),
mapped_variables=[:a4_8 => "E1" => :a4_8,]
),
MultiScaleModel(
model = Model13_E8(),
mapped_variables=[:a4_13 => "E1" => :a4_13,]
),
Status(a8_17 = 1.0, a8_18 = 1.0, ),
),
"E9" => (
MultiScaleModel(
model = Model12_E9(),
mapped_variables=[:a6_12 => "E1" => :a6_12,]
),
Status(),
),
"E10" => (
Status(),
),
"E11" => (
MultiScaleModel(
model = Model17_E11(),
mapped_variables=[:a8_17 => "E8" => :a8_17,]
),
MultiScaleModel(
model = Model18_E11(),
mapped_variables=[:a8_18 => "E8" => :a8_18,]
),
Status(),
),
)

outs = Dict("E1" => (:out1,:out2,:out4,:out6,:out9,:out11,:out14,),
"E2" => (:out10,),
"E3" => (:out3,),
"E4" => (:out19,),
"E5" => (:out5,:out16,),
"E6" => (:out15,),
"E7" => (:out7,),
"E8" => (:out8,:out13,),
"E9" => (:out12,),
"E10" => (),
"E11" => (:out17,:out18,),
)

meteo = Weather([
Atmosphere(T=25.0, Wind=1.0, Rh=0.6, Ri_PAR_f=200.0),
Atmosphere(T=10.0, Wind=0.5, Rh=0.6, Ri_PAR_f=200.0)
])

using MultiScaleTreeGraph

mtg = MultiScaleTreeGraph.Node(MultiScaleTreeGraph.NodeMTG("/", "E1", 0, 0),)

sim = run!(mtg, mapping, meteo, outputs = outs)

end
generated_test_1234_16_sim = generated_test_1234_16.run!(generated_test_1234_16.mtg, generated_test_1234_16.mapping, generated_test_1234_16.meteo, outputs = generated_test_1234_16.outs)